...
class Application(Frame):
    ...
    def createWidget(self):
        ...
        # 宽度 20 个字母(10 个汉字)，高度一个行高 self.w1.pack()
    ...
    def insertText(self):
        # INSERT 索引表示在光标处插入 self.w1.insert(INSERT, ' Gaoqi ')
        # END 索引号表示在最后插入 self.w1.insert(END, '[sxt]')
        self.w1.insert(1.8, "gaoqi")

    def returnText(self):
        # Indexes(索引)是用来指向 Text 组件中文本的位置，Text 的组件索引也是对应实际字符之间的位置。
        # 核心：行号以 1 开始 列号以 0 开始 print(self.w1.get(1.2, 1.6))
        print("所有文本内容：\n"+self.w1.get(1.0, END))
    ...
    def addWidget(self):
        ...
        # 在 text 创建组件的命令 self.w1.window_create(INSERT, window=b1)
